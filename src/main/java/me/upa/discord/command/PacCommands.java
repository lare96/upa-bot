...
public final class PacCommands extends ListenerAdapter {
    ...
    private static final String REDEEM_FOR_PORTFOLIO_ANALYSIS_INFO = "__**Rate**__: -> 500 PAC per analysis\n\n" +
            "__**How it works**__\n" +
            "<@373218455937089537> Will contact you with for additional details if needed, and send you the analysis as soon as it's ready.\n\n";
    ...
    public void onButtonInteraction(@NotNull ButtonInteractionEvent event) {
     ...
     event.getHook().setEphemeral(true).sendMessageEmbeds(new EmbedBuilder().
            ...
            case "confirm_redeem_for_portfolio_analysis":
                ...
                if (pacAmount < 500) {
                    event.reply("You need at least **500 PAC** to redeem this (You have **" + pacAmount + " PAC**).").setEphemeral(true).queue();
                    return;
                }
                ctx.discord().sendBotRequestsMsg(requester, "redeeming a treasure hunting map",
                        msg -> {
                            ctx.variables().storeRequests().accessValue(storeRequests -> storeRequests.putIfAbsent(msg.getIdLong(), new UpaStoreRequest(requester.getMemberId(), msg.getIdLong(), -500, "a portfolio analysis", RequestType.PAC)) == null);
                            event.getHook().setEphemeral(true).editOriginal("Your request to redeem a portfolio analysis has been sent to the UPA staff team. You'll hear from us shortly!").queue();
                        });
                event.getInteraction().editButton(event.getButton().asDisabled()).queue();
                break;
           ...
           //     event.getInteraction().editButton(event.getButton().asDisabled()).queue();
            ...
            case "redeem_for_portfolio_analysis":
                event.reply(REDEEM_FOR_PORTFOLIO_ANALYSIS_INFO).setEphemeral(true).addActionRow(
                        Button.of(ButtonStyle.SUCCESS, "confirm_redeem_for_portfolio_analysis", "Redeem a portfolio analysis", Emoji.fromUnicode("U+2705"))).queue();
            //    event.getInteraction().editButton(event.getButton().asDisabled()).queue();
                break;
                        ...
                        ctx.variables().storeRequests().access(sq -> {
                            ...
                            switch (request.getType()) {
                                ...
                                case PROPERTY:
                                    ...
                                    boolean isHollis = neighborhood != null && neighborhood.getName().equals("Hollis");
                                                ...
                                                public void onSuccess() {
                                                    ...
                                                    if (redeem) {
                                                        ...
                                                        if (poolProperty == null) {
                                                            ...
                                                            ctx.databaseCaching().getPoolProperties().put(propertyId, poolProperty);
                                                        }
    ...
    public void onModalInteraction(@NotNull ModalInteractionEvent event) {
        switch (event.getModalId()) {
                    ...
                    SqlConnectionManager.getInstance().execute(new SqlTask<UpaPoolProperty>() {
                        ...
                        if (result != null) {
                            ctx.discord().sendBotRequestsMsg(propertyFrom, "trading " + result.getDescriptiveAddress() + " for PAC",
                                    msg -> {
                                        ctx.variables().storeRequests().access(storeRequests -> storeRequests.get().putIfAbsent(msg.getIdLong(), new UpaStoreRequest(propertyFrom.getMemberId(), msg.getIdLong(), propertyId, "a property transfer to the UPA pool (" + success.getFullAddress() + ").", RequestType.PROPERTY)) == null);
            ...
            case "confirm_purchase_with_upx_form":
                ...
                event.deferReply(true).queue();
                ctx.discord().sendBotRequestsMsg(requester, "purchasing " + action,
                        msg -> {
                            long commentId = msg.getIdLong();
                            ctx.variables().storeRequests().access(map -> map.get().putIfAbsent(commentId, new UpaStoreRequest(requester.getMemberId(), commentId, amount, "through UPX transfer", RequestType.PAC)) == null);
            ...
            case "confirm_redeem_for_upx_form":
                ...
                event.deferReply(true).queue();
                ctx.discord().sendBotRequestsMsg(requester, "redeeming " + formattedAmt,
                        msg -> {
                            ctx.variables().storeRequests().access(storeRequests -> storeRequests.get().putIfAbsent(msg.getIdLong(), new UpaStoreRequest(requester.getMemberId(), msg.getIdLong(), -cost, "**" + amount + " UPX**", RequestType.PAC)) == null);
    ...
    private void handleDailyCommand(SlashCommandInteractionEvent event) {
        ...
        Instant lastReset = ctx.variables().lastDailyReset().getValue();
        if (lastClaimed.isBefore(lastReset)) {
            ...
            SqlConnectionManager.getInstance().execute(new SqlTask<Void>() {
                        ...
                        ctx.discord().sendCredit(new CreditTransaction(upaMember, Ints.checkedCast(dividend), CreditTransactionType.DAILY, null) {
                            @Override
                            public void onSuccess() {
            ...
            String claimIn;
            Instant now = Instant.now();
            Instant resetAt = DailyResetMicroService.wait(ctx.variables().lastDailyReset().getValue());
