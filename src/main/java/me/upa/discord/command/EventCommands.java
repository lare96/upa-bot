...
public final class EventCommands extends ListenerAdapter {
    ...
    public EventCommands(UpaBotContext ctx) {
        this.ctx = ctx;
    }
    ...
    private void handleSendStorm(SlashCommandInteractionEvent event) {
        if (ctx.eventProcessor().isEventActive()) {
            event.reply("Please end the current '" + ctx.eventProcessor().getEventName() + "' event before starting a new one.").queue();
            return;
        }
        ...
        ctx.discord().execute(() -> {
            ...
            UpaProperty property = ctx.databaseCaching().getProperties().get(propertyId);
            if (property == null) {
            ...
            Set<Member> members = participants.stream().map(next -> ctx.discord().guild().retrieveMemberById(next).complete()).collect(Collectors.toSet());
            ...
            var sendStormEvent = new SendStormEvent(ctx, participants, propertyId);
            sb.setLength(0);
            sb.append("**PREVIEW**\n\n").append(generateMessage(ctx, sendStormEvent));
            ...
            ctx.eventProcessor().setCurrentEvent(sendStormEvent);
        });
    }

    private void handleConfirm(ButtonInteractionEvent event) {
        ctx.eventProcessor().confirm(event);
    }

    public static StringBuilder generateMessage(UpaBotContext ctx, SendStormEvent event) {
        UpaProperty upaProperty = ctx.databaseCaching().getProperties().get(event.getPropertyId());
